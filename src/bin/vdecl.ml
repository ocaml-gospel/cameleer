...
module Translate = struct
  ...
  let rec core_type O.{ptyp_desc; _} = match ptyp_desc with
    | Ptyp_any ->
        assert false (* TODO *)
    | O.Ptyp_var _s ->
        assert false (* TODO *)
    | O.Ptyp_constr ({txt; _}, cty_list) ->
        let args = List.map core_type cty_list in
        mk_pttyapp (longident txt) args
    | O.Ptyp_arrow (Nolabel, _cty1, _cty2) ->
        assert false (* TODO *)
    | O.Ptyp_arrow (Labelled _, _, _) ->
        assert false (* TODO *)
    | O.Ptyp_arrow (Optional _, _, _) ->
        assert false (* TODO *)
    | O.Ptyp_tuple _cty_list ->
        assert false (* TODO *)
    | Ptyp_object _ ->
        assert false (* TODO *)
    | Ptyp_class _ ->
  ...
  let td_def td_manifest td_kind =
    let field_of_label_decl lbl_decl =
      let pld_mutable = lbl_decl.O.pld_mutable in

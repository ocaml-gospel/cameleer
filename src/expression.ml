...
and inner_pattern info P.{ppat_desc; ppat_loc; _} =
  ...
  let pat_desc = match ppat_desc with
    | P.Ppat_any    -> mk_pwild
    | P.Ppat_var id -> mk_pvar T.(mk_id ~id_loc:(location id.loc) id.txt)
    | P.Ppat_tuple pat_list ->
...
let mk_binder_info_none =
  mk_binder_info (T.dummy_loc) BNone
...
let binder_of_pattern =
  ...
  fun info P.{ppat_desc; ppat_loc; ppat_attributes; _} ->
    ...
    | Ppat_interval _ ->
        assert false (* TODO *)
    | Ppat_tuple pat_list -> incr counter;

...
module SeqPerm = struct

  (*@ function occ (s1: 'a seq) (v: 'a) (l u: integer) : integer *)

  (*@ axiom occ_base: forall s1: 'a seq, v l u.
        u <= l -> occ s1 v l u = 0 *)
...
type 'a array
(*@ ephemeral *)
(*@ mutable model contents2: 'a seq *)
(*@ model array_length: integer *)
(*@ invariant array_length = length contents2 *)

(*@ function elts (a: 'a array): 'a seq = a.contents2 *)
(*@ coercion *)

module Array = struct

  (* OCaml-like syntax a.(i) is mapped to Array.([_]) *)
  (*@ function ([_]) (a: 'a array) (i: integer) : 'a = Seq.get a.contents2 i *)

  (*@ function length (a: 'a array) : integer = Seq.len a.contents2 *)

end

module ArrayPermut = struct

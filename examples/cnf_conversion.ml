...
let[@logic] eval_literal (v : valuation) (f : literal) : bool =
  match f with FVar x -> v x | FConst b -> b
...
let[@lemma] rec size_nonneg_disj (phi : disj) =
  ...
  | FNeg_cnf _ | L_cnf _ -> ()
  | FOr_cnf (phi1, phi2) ->
      size_nonneg_disj phi1;
      size_nonneg_disj phi2
...
let[@lemma] rec size_nonneg_cnf (phi : formula_cnf) =
  match phi with
  | FAnd_cnf (phi1, phi2) ->
      size_nonneg_cnf phi1;
      size_nonneg_cnf phi2
...
let t (phi : formula) : formula_cnf = cnfc (nnfc (impl_free phi))
(*@ r = t phi

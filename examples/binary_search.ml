[@@@gospel {| predicate is_sorted (a: int array) =
...
module BinarySearch = struct
  let binary_search (compare: 'a -> 'a -> int) (a: 'a array) (v: 'a) =
    ...
    try while
      ...
      raise Not_found
    with Found i -> i
  [@@gospel {|i = binary_search_1 compare a v
...
module type OrderedType = sig
  ...
  [@@@gospel {| axiom is_pre_order_cmp: is_pre_order cmp |}]
end
...
= struct
  ...
  [@@@gospel {| function cmp (x: elt) (y: elt) : int = Ord.cmp x y |}]

  [@@@gospel {|predicate is_sorted (a: elt array) = forall i1 i2.

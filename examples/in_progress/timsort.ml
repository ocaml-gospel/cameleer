...
let[@lemma] rec sorted_mem (cmp: 'a cmp) (x: 'a) (l: 'a list) = match l with
  | [] -> ()
  | [_] -> ()
  | y1 :: ys -> sorted_mem cmp y1 ys
(*@ aux cmp x l
...
let[@lemma] rec sorted_rev_mem (cmp: 'a cmp) (x: 'a) (l: 'a list) = match l with
  | [] -> ()
  | [_] -> ()
  | y1 :: ys -> sorted_rev_mem cmp y1 ys
(*@ aux cmp x l
      requires is_pre_order cmp
...
let rec next_asc_run_rev compare revrun last len = function
  | x :: xs ->
      ...
      else let xs = x :: xs in len, Desc (last :: revrun), xs
...
let rec next_desc_run_rev compare (revrun: 'a list) last len = function
  | x :: xs ->
      ...
      else let xs = x :: xs in len, Asc (last :: revrun), xs

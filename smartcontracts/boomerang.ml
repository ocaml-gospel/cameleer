...
let [@entry] main env () () =
  ...
  let ops =
    if Tz.eq amount (Tz 0) then []
      ...
      let source_contract = Option.get (Contract.contract (Global.get_source env) RepUnit) in
...
(*@ ops, stg = main env param storage
      ensures   if Tz.eq (Global.get_amount env) (Tz 0) then ops = []
                  ...
                  match Contract.contract (Global.get_source env) RepUnit with
                  | Some sc -> ops = Operation.transfer_tokens ParamUnit (Global.get_amount env) sc :: []
                  | None -> true
      raises Invalid_argument _ ->
        match (Contract.contract (Global.get_source env) RepUnit) with None -> true | Some _ -> false *)
